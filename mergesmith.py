...
for foo in persents:
   ...
   sawents = 1 + sawents
   args = foo.split('\t')
   curcit1 = re.sub('([0-9]+\\.[0-9]+\.[0-9]+).*','Thuc. ' + '\g<1>	',args[0])
   curcit2 = re.sub('([0-9]+\\.[0-9]+).*','Thuc. ' + '\g<1>	',args[0])
   #print('cits',args[0],curcit1,curcit2,sep='-')
   for boo in toposents:
     #if( re.search(r'Thuc. 2\.1',boo)):
     #print("cit2",curcit2,boo)
     if( re.search(curcit1,boo) or re.search(curcit2,boo)):
       a2 = boo.split()
       xlit = persents[foo]
       ...
       if( foo in besthits ):
         if(r > besthits[foo] ):
           besthits[foo] = r
           bestlinks[foo] = boo
           #print("hit2",r,xlit,a2[4],curcit,boo)
       else:
         besthits[foo] = r
         bestlinks[foo] = boo
...
for tmp in bestlinks:
   ...
   if( besthits[tmp] < 71 and not  re.search('^' + c1 ,args[3])):
       print( besthits[tmp],'reject',persents[tmp],args[3],tmp,bestlinks[tmp])
   else:
       allthucents[tmp] = 2
       print(besthits[tmp],'keep',persents[tmp],args[3],tmp,bestlinks[tmp])
