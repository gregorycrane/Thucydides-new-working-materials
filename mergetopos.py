...
for foo in persents:
   ...
   sawents = 1 + sawents
   args = foo.split('\t')
   curcit = re.sub('([0-9]+\.[0-9]+).*','^\g<1> ',args[0])
   for boo in toposents:
     if( re.search(curcit,boo)):
       a2 = boo.split()
       xlit = persents[foo]
       r = fuzz.ratio(a2[4],xlit)
       if( foo in besthits ):
         if(r > besthits[foo] ):
           besthits[foo] = r
           bestlinks[foo] = boo
...
for tmp in bestlinks:
   ...
   if( besthits[tmp] < 71 and not  re.search('^' + c1 ,args[4])):
       print( besthits[tmp],'reject',persents[tmp],tmp,bestlinks[tmp])
   else:
       print(besthits[tmp],'keep',persents[tmp],tmp,bestlinks[tmp])
